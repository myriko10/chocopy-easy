...
def run_game():
    ...
    frame_counter = 0
    state = 1
    step_on_frame = IMAGE_DICT['red'].get_height(
    ) / Hurdle.speed  # 着地の時に踏んでしまう可能性のあるフレーム数の計算
    # 難易度調整用マージン(係数2と3で大分体感が変わる)
    COLLISION_MARGIN = IMAGE_DICT['red'].get_width() * 3
    collision_area = (player.image.get_width() +
                      step_on_frame + COLLISION_MARGIN) / Hurdle.speed
